 ...
 * @author Lama1
 */
public class SolutionLibrarySorter7 extends SolutionBase {
    ...
    public SolutionLibrarySorter7(String id, LibraryInfoSorter libraryInfoSorter, int filter, int noOfCandidates, 
            int scoreToBeat, int top, int sequenceNo)
    {
        super(App.Name + "_SolutionLibrarySorter7");       
        this.id = id;
            ...
            for(var lib : libraryList)
            {
                ...
                for(int d = 1; d <= daysLeftForThisLibrary; d++)
                {
                    ...
                    for(int p = 1; p <= lib.ShipPerDay; p++)
                    {
                        ...
                        var book = lib.Books.get(lb);
//                        if(!booksShipped.contains(book))
//                        {
                            lib.MaxScore += book.Score;
//                        }
                        lb++;
                    }
                    if(ldone)
